...
class ClientThread(Thread):
    ...
    def login(self):
        ...
        if name in USERS.keys():
            ...
            if USERS[name]['пароль'] == get_password_hash(self.receive_msg()):
                self.success_login()
            else:
                self.close_connection('неверный пароль')
        else:
            self.send_msg('Установите новый пароль')
            USERS.update({name: {'пароль': get_password_hash(self.receive_msg())}})
            save_users()

    def success_login(self):
        self.send_msg(f'Успешное соединение')
        save_users()
    ...
    def receive_msg(self):
        ...
        try:
            ...
            self.close_connection('ошибка подключения')

    def run(self):
        CONNECTIONS_LIST.append(self)
        self.send_msg(f'{self.username}, добро пожаловать!')
        service_msg(self, 'вы подсоединены')
...
logging.basicConfig(format="%(asctime)s [%(levelname)s] %(funcName)s: %(message)s",
                    handlers=[logging.FileHandler("logs/server.txt"), logging.StreamHandler()], level=logging.INFO)
